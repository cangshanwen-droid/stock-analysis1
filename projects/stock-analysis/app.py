...
def secret_value(name, default=""):
    ...
    try:
        ...
        logging.warning("Failed to read secret '%s', using default", name)
        return default
...
def restore_db_from_backup_if_missing():
    ...
    try:
        ...
        logging.warning("Database restore from backup failed", exc_info=True)
...
def persist_db_backup(reason=""):
    ...
    try:
        try:
            ...
            logging.warning("WAL checkpoint failed, continuing backup", exc_info=True)
        ...
        logging.warning("Database backup failed", exc_info=True)
        return False
...
def get_admin_password():
    """从环境变量或 Streamlit secrets 读取管理员密码；未设置时生成随机密码"""
    ...
    try:
        ...
        logging.warning("Failed to read admin password from secrets, falling back to generated token")
    pw = secrets.token_urlsafe(16)
    print(f"[SECURITY] No ADMIN_PASSWORD set. Generated random recovery password: {pw}", file=sys.stderr)
    return pw
...
def init_db():
    with get_db_cm() as conn:
        ...
        try:
            ...
            # Check if column already exists before logging warning
            cols = [row[1] for row in conn.execute("PRAGMA table_info(stocks)").fetchall()]
            if "revenue" not in cols:
                logging.warning("ALTER TABLE ADD COLUMN revenue failed", exc_info=True)
...
def auth_user(u, p):
    ...
    try:
        ...
        logging.warning("Failed to check user status for '%s'", u, exc_info=True)
        return False, ""
    return True, r["role"]
...
def register_user(u, p, role="player"):
    try:
        ...
        logging.warning("Error registering user '%s'", u, exc_info=True)
        return False, "注册失败"
...
def add_stock(sym, name, total_shares, revenue, industry_pe):
    try:
        ...
        except: logging.warning("Failed to clear get_stocks cache", exc_info=True)
        try: get_public_quote_snapshot.clear()
        except: logging.warning("Failed to clear get_public_quote_snapshot cache", exc_info=True)
        return True, f"添加成功，初始价={price}"
    except sqlite3.IntegrityError:
        return False, "代码已存在"
    except Exception:
        logging.warning("Error adding stock '%s'", sym, exc_info=True)
        return False, "添加失败"
...
def close_market():
    ...
    try:
        ...
        logging.warning("Failed to clear get_public_quote_snapshot cache in close_market", exc_info=True)
    try: get_stocks.clear()
    except: logging.warning("Failed to clear get_stocks cache", exc_info=True)
    try: is_market_open.clear()
    except: logging.warning("Failed to clear is_market_open cache", exc_info=True)
    try: get_market_round.clear()
    except: logging.warning("Failed to clear get_market_round cache", exc_info=True)
...
def open_market():
    ...
    try:
        ...
        logging.warning("Failed to clear get_public_quote_snapshot cache in open_market", exc_info=True)
    try: get_stocks.clear()
    except: logging.warning("Failed to clear get_stocks cache", exc_info=True)
    try: is_market_open.clear()
    except: logging.warning("Failed to clear is_market_open cache", exc_info=True)
...
def undo_market():
    ...
    try:
        ...
        logging.warning("Failed to clear get_public_quote_snapshot cache in undo_market", exc_info=True)
    try: get_stocks.clear()
    except: logging.warning("Failed to clear get_stocks cache", exc_info=True)
    try: is_market_open.clear()
    except: logging.warning("Failed to clear is_market_open cache", exc_info=True)
...
def reset_to_round1():
    ...
    try:
        ...
        logging.warning("Failed to clear get_public_quote_snapshot cache in reset_to_round1", exc_info=True)
    return 1
                ...
                for i in range(len(df)):
                    if pd.notna(df.iloc[i, 0]) and pd.notna(df.iloc[i, 1]):
                        try:
                            ...
                            logging.warning("Failed to parse row %d in batch stock import: %s", i, str(df.iloc[i].to_dict()), exc_info=True)
